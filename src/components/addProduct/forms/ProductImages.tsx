...
const ProductImages = () => {
  ...
  function removeData(index: number) {
    setAllFiles(allFiles.filter((elem: any, ind: number) => {
      return ind !== index
    }))
  }
    ...
    <form className="std-card m-2">
      ...
      <div className="row m-2">
        ...
        <div className='input-group'>
          <input type="file" className="std-inputField" placeholder='select file' onChange={(e: any) => { setFile(e.target.files[0]) }}></input>
          <div className='std-btn std-btnOrange' onClick={() => { addFile() }}>+</div>
        </div>
      </div>
      <br />
      <div>
