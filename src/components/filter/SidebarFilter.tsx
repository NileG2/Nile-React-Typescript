...
const SidebarFilter = (props: any) => {
    ...
    const allQueryProducts = props.queryProducts;
    const allSearchProducts = props.searchProducts;

    //has some bugs
    const handleCatSelect = (e: React.ChangeEvent<HTMLInputElement>): void => {
        if (val.includes(e.target.value)) {
            const newVal = val.filter((x) => x !== e.target.value);
            setVal([...newVal]);
        } else {
    ...
    useEffect(() => {
        const handleDeptFilter = (): void => {
            ...
            if (props.operation === "query") {
                const all = [...props.queryProducts];
                const filtered = all.filter((prod: any) => val.includes(prod.category));
                props.setQueryProducts(filtered);
            }

            if (props.operation === "search") {
                const all = [...props.searchProducts];
                const filtered = all.filter((cat: any) => val.includes(cat.category));
