...
const PlaceOrder = () => {
  ...
  const [availableCoupons, setAvailableCoupons] = useState([
    {
      ...
      details: {
        amount: "1000",
        description: "Get flat 1000/- off on a purchase of 16999/-  and above using GPay",
        code: "GLoyal"
      }
    }, {
      type: "discount",
  ...
  function getSubTotal() {
    if (currCoupon !== null) {
      ...
      return dummyCartTotal + dummyDeliveryCharge - (dummyCartTotal * currCoupon.details.amount)
    }
          ...
          <div className='col'>
            <div className='m-2'>
              <p className='std-font2'>Appled Coupon</p>
              {
                currCoupon !== null ?
                  <div>
                    <button className='std-btn' onClick={() => { setCurrCoupon(null) }}>remove</button>
                    <Coupon coupon={currCoupon} />
                  </div> : <>No Coupon Selected</>
              }
            </div>
          </div>
          <div className='col'>
            <div>
              ...
              {
                currCoupon !== null ? <div className='row'>
                  <div className='col'>
                    <p className='std-boldFont p-2 m-0'>Coupon Discount</p>
                  </div>
                  <div className='col d-flex'>
                    {
                      currCoupon.type === 'flat' ? <p className='std-font2 std-greenText m-0'>-{currCoupon.details.amount}</p> :
                        <p className='std-font2 std-greenText m-0'>-{printPrecision()}</p>
                    }
            ...
            <div className='d-flex justify-content-center'>
              <button className='std-btn std-btnOrange' style={{ width: "20rem" }} onClick={(e) => { }}>Pay</button>
            </div>
          </div>
        </div>
          ...
          <div className='d-flex flex-wrap'>
            {
              availableCoupons.map((coupon, index) => {
                return <div key={index} onClick={() => { handleSelectCoupon(index) }}>
